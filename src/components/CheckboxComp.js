    ...
    return(
      <div 
        style={{pointerEvents:`${this.props.clickCount > 4 &&
                                 this.state.checked ==
                                 false ? 'none' : 'auto'}`,
                background:`${this.state.checked ? 'rgba(61, 75, 105, .8)' : 'rgba(61, 75, 105, .3)'}`}}
        ...
        <label 
          style={{
                  ...
                  !this.state.checked && this.props.clickCount > 4 ? 'rgba(61, 75, 105, .3)' :
                  '#000'}`}}>
          {this.props.catName}
        </label>
        <input 
          disabled={this.props.clickCount > 4 && this.state.checked == false ? true : false}
          type="checkbox" 
          checked={this.state.checked}
        />
      </div>
